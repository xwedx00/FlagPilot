...
# FlagPilot adaption: relative imports
from lib.tools.tool_convert import (
    convert_code_to_tool_schema,
    convert_code_to_tool_schema_ast,
    function_docstring_to_schema
)
from lib.tools.tool_data_type import Tool, ToolSchema

# FlagPilot adaptation: we don't have metagpt.const.TOOL_SCHEMA_PATH
# We'll use a local path
TOOL_SCHEMA_PATH = Path(__file__).parent / "schemas"
...
class ToolRegistry(BaseModel):
    ...
    def register_tool(
        ...
        # We don't necessarily need to write yml files for dynamic tools
...
def register_tools_from_file(file_path) -> dict[str, Tool]:
    ...
    for name, schemas in tool_schemas.items():
        ...
        # register_tool above adds to TOOL_REGISTRY global
